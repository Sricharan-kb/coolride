 ...
 * sampling, live stats, and persistence. Geolocation/sensors/weather live here
 * (not in App) because they are gated on `isTracking`, which derives from this
 * hook's own `rideState` — so they must run inside it to avoid a render-time
 * ordering cycle.
 */
